...
const REASONS = [
  ...
  {
    // 🚨 '온라인 판매 안 함'이라고 쓰지 않는다 (2026-08-25 대표님 지적) —
    //    우리 자사몰(57tb.art)이 주문 시스템을 갖추고 열릴 예정이라 그 순간 거짓이 된다.
    //    실제로 막는 것은 **오픈마켓(쇼피·라자다·틱톡)** 이고, 그게 세리가 문제 삼는 지점이다.
...
// 🚨 '④ 주문' 단계를 두지 않는다 (2026-08-25 대표님).
//    체험 신청 버튼을 누르기 전에 결제를 떠올리게 하면 무료 체험의 문턱이 스스로 높아진다.
//    주문 안내는 체험 뒤에 개별로 한다 — 화면이 먼저 "그다음엔 사세요"라고 말하지 않는다.
const STEPS = [
