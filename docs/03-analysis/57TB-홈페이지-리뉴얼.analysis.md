...
- `GalleryStyle` type alias (`'Digital Perm' | 'S Perm' | 'Volume Magic' | 'Mix Perm' | 'Hair Color'`) -- added 2026-02-11 *(`'Balayage'` 제거 2026-06-20: 발레야쥬 서비스 종료)*
...
- [x] Gallery images reorganized into 6 category-based folders (`digital-perm/`, `color/`, `s-perm/`, `volume-magic/`, `mix-perm/`, `balayage/`) *(balayage/ 폴더는 2026-06-20 발레야쥬 종료로 삭제 → 현재 5개)*
...
- [x] Gallery filter order set: Hair Color → Volume Magic → Digital Perm → Mix Perm → S Perm *(→ Balayage는 2026-06-20 발레야쥬 종료로 제거)*
...
| 1.2 | 2026-07-08 | BELLISTA 리테일 필드 신설 `/products` (unlisted: 메뉴·sitemap 미등록+noindex) — 목록(Scalp Care Line 6 + Hair Perfume Line 9) + 상세 15종(`/[locale]/products/[slug]`, 검수 확정 태국어 상세 WebP 조각). 데이터 정본 `src/data/products.ts`(slug=QR 인쇄용, 변경 금지), 규칙은 CLAUDE.md "/products" 섹션 참조 | Dylan Kwak |
